            ...
            for s in prange(m, num_threads=t):
                if L_keep[s] == 1:
                    ...
                    for Aa in range(Dl+1):
                        ...
                        W_l[s] = W_l[s] + f_gl_log * P_gl0 * P_r_a0 * AD_summary_dict[iter_key][1][0]
                        W_l[s] = W_l[s] + f_gl_log * P_gl1 * P_r_a1 * AD_summary_dict[iter_key][1][1]
                        W_l[s] = W_l[s] + f_gl_log * P_gl2 * P_r_a2 * AD_summary_dict[iter_key][1][2]
